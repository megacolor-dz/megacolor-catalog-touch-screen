...
do
  ...
  cat_anchor_html=$(cat anchor-fancybox-template.html | sed "s/<%PATH%>/images\/$d\/${cat_img}/" | sed "s/<%DESCRIPTION%>/${cat_name}/" | sed "s/<%TARGET%>/${cat_name}.html/" | sed "s/<%NAME%>/${cat_name}/" )
